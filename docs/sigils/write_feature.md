...
- **leaf vs. non-leaf** — the recursion question. For each affordance I discover, I ask: can this be implemented in one function with no design decisions left open? If yes, it is a leaf. If it requires multiple design decisions or hides a coordinate system, it gets its own subfolder and its own feature document.
